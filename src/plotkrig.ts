...
const costs: {[op: string]: number} = {
    'constant': 1, // Cost for each constant
    'char': 0, // Cost for each char in constant
    '+': 1,
    '-': 1,
    '*': 1,
    '/': 1,
...
const elms = {
    ...
    roomname:   <HTMLInputElement>document.getElementById("roomname"),
    isServer:   <HTMLInputElement>document.getElementById("isServer"),
    ...
    xSlider:    <HTMLInputElement>document.getElementById("xSlider"),
    ySlider:    <HTMLInputElement>document.getElementById("ySlider"),
    readyButton:document.getElementById("readyButton")!,

    expression:     <HTMLInputElement>document.getElementById("expression"),
    lengthInput:    <HTMLInputElement>document.getElementById("lengthInput"),
...
var bufferCanvas = document.getElementById("bufferCanvas") as HTMLCanvasElement;
                ...
                if(isServer){
                    if(plots[i].settings.projectileType == "Regular" || plots[i].settings.projectileType == "Explosive"){
                        ...
                        if(plots[i].length >= plots[i].maxLength * plots[i].settings.fuse){
                            ...
                            sendTerrainRect(
                                x*cfg.scale + cfg.hw - Math.cos(angle) * cfg.projectiles.wallWidth - 5, y*cfg.scale + cfg.hh - Math.sin(angle) * cfg.projectiles.wallWidth - 5,
                                Math.cos(angle) * 2 * cfg.projectiles.wallWidth * cfg.scale + 10, Math.sin(angle) * 2 * cfg.projectiles.wallWidth * cfg.scale + 10
                            );
                            updateTerrainImage();
                            stop = true;
                        }
...
var peer_id: number | null= null;
var conn: DataConnection | null = null;

var clients: {[key: string]: {
    readyState: ReadyState,
    conn: DataConnection
}} = {};

var players: {[key: string]: {
    x: number,
    y: number,
    team: number,
    alive: boolean
...
var teams: {money: number, cap: number, perTick: number, capPerTick: number}[] = [];

var plots : {
    name: string,
    expression: math.EvalFunction,
    derivative: number,
...
var terrain: ImageData | null = null;
var terrainImage: ImageBitmap | null = null;

var timeSinceCurrencyGiven : number;
var startTimestamp: number;
var generationIncreases: number;
var targets: Target[] = [];
            ...
            conn.on("data", d=>{ // .t - type, .d - data
                ...
                switch(data.t){
                    ...
                    case "terrain": // Deprecated
                        if(terrain == null) terrain = new ImageData(cfg.w, cfg.h);
...
async function start(){
    if(elms.isServer.checked){
        ...
        bctx.save();
        bctx.scale(-1, 1);
        bctx.drawImage(terrainImage as CanvasImageSource, 0, 0, cfg.w / 2, cfg.h, -cfg.w, 0, cfg.w/2, cfg.h);
        bctx.restore();
        terrain = bctx.getImageData(0, 0, cfg.w, cfg.h);
        await updateTerrainImage();
...
function addTarget(x1: number, x2: number, size: number, effect: string, op: string, amount: number, contestable: boolean){
    while(true){
        let target = new Target(
            Math.random()*(x2-x1) + x1,
            (Math.random()-0.5)*cfg.h/cfg.scale,
            size,
            effect,
            op,
            amount,
...
function collectSettings(){
    ...
    } as ProjectileSettings
}

function tryPlot(name: string, expression: string, derivative: number, settings: ProjectileSettings){
...
class Target{
    ...
    clone(): Target{
        return new Target(
            this.x, this.y, this.size, this.effect, this.op, this.amount, this.contestable
        );
    }
}
...
// ---- Helper functions ----
//Graphics
function getPixel(x: number, y: number, channel: number){
...
function collideCircleSimple(x: number, y: number, r: number){
...
async function updateTerrainImage(){
    terrainImage = await createImageBitmap(terrain!);
}
...
function sendTerrainRect(x: number, y: number, w: number, h: number){
    x = Math.floor(x);
    y = Math.floor(y);
...
function getCost(expression: string, settings: ProjectileSettings) :{err: string | null, cost: number | null} | undefined {
    expression = fixExpression(expression);
    let tree: math.MathNode;
    try{
        ...
        tree.traverse((node, path, parent) => {
            switch(node.type){
                case "ConstantNode":
                    let value = (node as math.ConstantNode).value;
                    if(value != undefined){
                    ...
                    break;
                case "SymbolNode":
                    let name = (node as math.SymbolNode).name;
                    ...
                    break;
                case "OperatorNode":
                    let op = (node as math.OperatorNode).op;
...
function setPosition(pos: {x: number, y: number}, name: string){
